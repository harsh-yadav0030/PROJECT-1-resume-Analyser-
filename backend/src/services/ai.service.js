...
import { anonymizeResume } from "./privacy.service.js";
import { restoreResume } from "../utils/privacy/restoreResume.js";
...
async function generateResumePdf({ resume, selfDescription, jobDescription }) {
    ...
    const privacyResult = anonymizeResume(resume);
    ...
    const resumePdfSchema = z.object({
     ...
     const prompt = ` Generate resume for a candidate with the following details.
                       IMPORTANT:
                       Some contact information has been anonymized using placeholders such as [EMAIL_1] and [PHONE_1].
                       Preserve these placeholders exactly as provided.
                       Do not rename, modify, or remove them.
    ...
    const restoredHtml = restoreResume(
       jsonContent.html,
       privacyResult.mapping
    );

    const pdfBuffer = await generatePdfFromHtml(restoredHtml);
